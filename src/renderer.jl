...
function generateVertexData(renderer::FontRenderer, text::String, xPos::Float32 = 10.0f0, yPos::Float32 = 50.0f0)
    ...
    push!(renderer.vertices, BufferVertex(textBlockLeft, renderer.windowHeight - textBlockBottom, 0.9f0, 0.0f0, 0.0f0, -2))  # Bottom-left
    push!(renderer.vertices, BufferVertex(textBlockRight, renderer.windowHeight - textBlockBottom, 0.9f0, 0.0f0, 0.0f0, -2))  # Bottom-right
    push!(renderer.vertices, BufferVertex(textBlockLeft, renderer.windowHeight - textBlockTop, 0.9f0, 0.0f0, 0.0f0, -2))  # Top-left
    
    # Second triangle: bottom-right, top-right, top-left (counter-clockwise)
    push!(renderer.vertices, BufferVertex(textBlockRight, renderer.windowHeight - textBlockBottom, 0.9f0, 0.0f0, 0.0f0, -2))  # Bottom-right
    push!(renderer.vertices, BufferVertex(textBlockRight, renderer.windowHeight - textBlockTop, 0.9f0, 0.0f0, 0.0f0, -2))  # Top-right
    push!(renderer.vertices, BufferVertex(textBlockLeft, renderer.windowHeight - textBlockTop, 0.9f0, 0.0f0, 0.0f0, -2))  # Top-left
    ...
    for (wordIndex, word) in enumerate(words)
        ...
        for char in word
            ...
            if haskey(glyphs, char)
                ...
                x1 = xOffset + bearingX
                y1 = renderer.windowHeight - (yOffset + bearingY - height)  # Flipped Y for WebGPU  
                x2 = x1 + width
                y2 = renderer.windowHeight - (yOffset + bearingY)           # Flipped Y for WebGPU
...
# Simple layout engine function for positioning text at specific coordinates
function setPosition(renderer::FontRenderer, text::String, x::Float32, y::Float32)
